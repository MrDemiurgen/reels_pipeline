...
# =========================
# CARD GEOMETRY CALCULATION
# =========================

CARD_W = int(VIDEO_W * CARD_SCALE)     # вычисляем ширину карточки
CARD_H = int(CARD_W * 9 / 16)          # высота карточки (соотношение 16:9)

CARD_X = (VIDEO_W - CARD_W) // 2       # центрируем карточку по горизонтали

# вычисляем высоту блока из трёх карточек
TOTAL_BLOCK_HEIGHT = CARD_H * 3 + CARD_GAP * 2

# центрируем весь блок карточек по вертикали
...
def make_frame_factory(background: Image.Image, cards: List[CardPair]):
...
def build_video(input_dir: str, background_path: str, music_path: str, output_path: str) -> None:
    ...
    make_frame = make_frame_factory(background, cards)
